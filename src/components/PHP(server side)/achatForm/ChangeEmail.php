<?php
  ...
  if(!empty($old_email) && !empty($new_email) && !empty($password)){
    if(filter_var($old_email, FILTER_VALIDATE_EMAIL) && filter_var($new_email, FILTER_VALIDATE_EMAIL)){
      $sql = mysqli_query($conn, " SELECT * FROM `admin` WHERE `admin_email` = '{$old_email}' ");
      if(mysqli_num_rows($sql) > 0){
        $row = mysqli_fetch_assoc($sql);
        if($old_email === $row['admin_email'] && password_verify($password, $row['admin_pass'])){
          $sql2 = mysqli_query($conn, " UPDATE `admin` SET `admin_email` = '{$new_email}' ");
          echo "emailChanged";
