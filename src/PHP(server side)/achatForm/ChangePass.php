<?php
  ...
  $new_hashed_pass = password_hash($newPassword, PASSWORD_DEFAULT); //hash password

  if(!empty($myemail) && !empty($mypassword) && !empty($newPassword)){
    if(filter_var($myemail, FILTER_VALIDATE_EMAIL)){
      $sql = mysqli_query($conn, " SELECT * FROM `admin` WHERE `admin_email` = '{$myemail}' ");
      if(mysqli_num_rows($sql) > 0){
        $row = mysqli_fetch_assoc($sql);
        if($myemail === $row['admin_email'] && password_verify($mypassword, $row['admin_pass'])){
